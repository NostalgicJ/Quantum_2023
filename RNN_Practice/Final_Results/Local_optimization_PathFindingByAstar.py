...
filename = "/Local_Optimization_ByAstar_dt_" + str(dt) + "_" + printdate + '.csv'
...
# upload existed file - Theta, Phi, Combination
...
while True : 
    ...
    df_comb = sample["combination"][case]
    df_fid = sample["fidelity"][case]
    
    if comb != [] or comb:
        path, computing_time = main(df_theta, df_phi, df_comb)
    output = [['case' + str(case), len(path), df_theta, df_phi, dt, path, len(path)*dt, computing_time]]
