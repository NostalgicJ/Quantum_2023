...
def Astar_Qsearch(init, target):
    ...
    while openList:
        ...
        if iteration < 10: 
            fid = 0.9999
        elif iteration < 3000:  # 해당 iteration 범위 설정하여 long하게 탐색하도록 변경 가능(더 많은 경우의 수 탐색)
            fid = 0.99
        else:
            return [-1] 
...
filename = "/ByAstar_Rho_version_H3_Omega_part_" + printdate + '.csv'
...
while True : 
    ...
    # 원하는 범위(결측 구간)에서 long하게 찾음
    # 아래 폼 변경하여 범위 설정 가능
    if np.random.rand() < 0.5:
        theta = np.arccos(np.random.uniform(-0.0292, -0.5048))  # [1.6, 2.1)에서 랜덤한 값
    else:
        theta = np.arccos(np.random.uniform(0.2675, 0.5403))    # [1.0, 1.3)에서 랜덤한 값
    # omega값 추가
